defmodule DgraphEx.Core.Query.BlockTest do
  use ExUnit.Case, async: true
  doctest DgraphEx.Core.Query.Block
  import TestHelpers

  import DgraphEx

  test "func function renders" do
    assert render(func(:person, eq(:name, "bleep"))) ==
             clean_format("""
  ...
  test "block with keyword multiple term alias (at the end only)" do
    assert render({
             :name,
             :address,
             age: :years, favorite_color: :fav_color
           }) ==
             clean_format("""
               {
                 name
  ...
  test "block with expressions as keywords works" do
    assert render({
             :name,
             :address,
             age: count(:years), friends: sum(val(:F))
           }) ==
             clean_format("""
               {
                 name
