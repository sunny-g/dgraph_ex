defmodule DgraphEx.Core.Kwargs do
  @moduledoc false
  ...
  def parse(kwargs) when is_list(kwargs), do: do_parse(%Query{}, kwargs)

  defp do_parse(q, []), do: q

  defp do_parse(_q, [{:set, %_{} = model} | rest]) do
    model
    |> DgraphEx.set()
    |> do_parse(rest)
  end
  ...
  defp do_parse(_q, [{:as, subject}, {:set, %_{} = model} | rest]) do
    DgraphEx.set(subject, model)
    |> do_parse(rest)
  end

  defp do_parse(q, [{:as, key}, {:get, %Query{} = get_q} | rest])
       when is_atom(key) do
    q
    |> DgraphEx.as(key)
    |> Query.merge(get_q)
  ...
  # as => key => get => func
  defp do_parse(q, [{:as, key}, {:get, :var}, {:func, _} = fun | rest])
       when is_atom(key) do
    q
    |> DgraphEx.as(key)
  ...
  # invalid :as follower
  defp do_parse(_, [{k, v} | _])
       when (k == :as or v == :as) and (is_atom(k) and is_atom(v)) do
    raise %ArgumentError{
      message:
        "When building a query :as can only be followed by `get: :var` or `func: <expr>` keywords"
    }
  end
  ...
  defp do_parse(%Query{sequence: [%Block{} = b | rest_seq]} = q, [{executor, expr} | rest])
       when executor in @executors do
