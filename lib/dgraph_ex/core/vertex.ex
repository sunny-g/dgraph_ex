defmodule DgraphEx.Core.Vertex do
  @moduledoc false

  alias DgraphEx.Core.{Expr, Field, Query, Vertex}
  alias DgraphEx.Util
  alias Expr.Uid
  alias Query.Select
  ...
  def as_setter(subject, %{__struct__: _} = model) do
    subject
  ...
  def as_selector(%_{} = model) do
    ...
    |> Enum.map(fn
      {key, %_{} = submodel} -> {key, Select.new(submodel)}
      {key, _} -> {key, nil}
    end)
  end
  ...
  def is_model?(module)
      when is_atom(module),
      do: Util.has_function?(module, :__vertex__, 1)
