defmodule DgraphEx.Core.Changeset do
  @moduledoc false

  alias DgraphEx.Core.Changeset, as: Cs
  alias DgraphEx.Core.{Field, Vertex, Types}
  alias DgraphEx.Util
  ...
  After checking for a non-list errors field, we check valid?/1 which returns true only for empty
  ...
  def uncast(%Cs{} = cs) do
    cond do
      !is_list(cs.errors) ->
        raise %ArgumentError{
          message:
            "A DgraphEx Changeset requires the :errors field to be a keyword list. Got #{
              inspect(cs.errors)
  ...
  def valid?(%Cs{errors: []}), do: true
  def valid?(%Cs{}), do: false
