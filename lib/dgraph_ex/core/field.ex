defmodule DgraphEx.Core.Field do
  @moduledoc false

  alias DgraphEx.Core.{Alter, Delete, Expr, Field, Query, Set, Vertex}
  alias Expr.{Uid}
  ...
  def new(predicate, type, options \\ [])
    ...
    %Field{
      ...
      count: Keyword.get(options, :count) && true,
      default: options[:default],
      virtual: Keyword.get(options, :virtual) && true,
  ...
  def put_subject(fields, subject)
      when is_list(fields),
      do: Enum.map(fields, &put_subject(&1, subject))

  def put_subject(%Field{} = field, subject)
      when is_atom(subject),
      do: %{field | subject: subject}
